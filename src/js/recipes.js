...
document.getElementById('filter-btn').addEventListener('click', function () {
    ...
    if (selectedArea) {
        ...
        fetch(apiURL)
            ...
            .catch(error => console.error('Error reloading recipes:', error));
    }
});
...
function recipeMatchesCategory(recipe) {
    ...
    return fetch(recipeDetailURL)
        ...
        .catch(error => {
            console.error('Error verifying recipe category:', error);
            return false;
        });
}
...
function showNoResultsMessage(area) {
    ...
    recipeGrid.innerHTML = `<p>No recipes found for the area "${area}" in the selected category.</p>`;
}
                ...
                .then(data => {
                    if (data.meals) {
                        ...
                        if (filteredRecipes.length > 0) {
                            ...
                            showNoResultsMessage(`"${searchTerm}" in the selected category`);
                        }
                    } else {
